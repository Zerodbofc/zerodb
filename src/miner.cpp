...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if(prev->nHeight <= 9) { txNew.vout[0].nValue = 250000 * COIN;} // Premine 210,000 * 10 = 210,000 zerodb 4.2%
    if (prev->nHeight > 9 && prev->nHeight <= 300) { txNew.vout[0].nValue = 333,33 * COIN;} // Block 9 to 300 pay 1 to prevent instamine
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("zerodbMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("zerodbMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("zerodbMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zerodb-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running zerodbMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
