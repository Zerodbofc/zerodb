...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::zerodb)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("zerodb")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizezerodbAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizezerodbAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizezerodbAmount = strAnonymizezerodbAmount.remove(strAnonymizezerodbAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizezerodbAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizezerodbAmount * COIN) nMaxToAnonymize = nAnonymizezerodbAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizezerodbAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizezerodbAmount));
        strAnonymizezerodbAmount = strAnonymizezerodbAmount.remove(strAnonymizezerodbAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizezerodbAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
